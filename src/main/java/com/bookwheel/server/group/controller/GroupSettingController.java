...
public class GroupSettingController {
    ...
    @Operation(
            summary = "모임 삭제",
            description = "ACTIVE 모임장(LEADER)만 모집 중(RECRUITING) 또는 완료(COMPLETE) 모임을 DELETED 상태로 전환할 수 있습니다. 멤버·채팅·일정·알림은 정리하지만 게시물과 게시물 이미지는 보존합니다. 진행 중(IN_PROGRESS) 모임은 삭제할 수 없습니다."
    )
    @DeleteMapping
    ...
    @Operation(
            summary = "멤버 강퇴",
            description = "ACTIVE 모임장(LEADER)만 그룹의 멤버를 강제 탈퇴시킬 수 있습니다. 모집 중에는 PLANNED 배정을 다시 판단합니다. " +
                    "진행 중에는 기존 미래 라운드를 보존·비활성화하고 읽기 순서 재확인 상태로 전환합니다."
    )
    ...
    @Operation(summary = "부모임장 등록/해제", description = "ACTIVE 모임장(LEADER)만 그룹 멤버의 부모임장 권한을 부여하거나 해제할 수 있습니다.")
    ...
    @Operation(summary = "모임장 위임", description = "현재 ACTIVE 모임장(LEADER)만 다른 ACTIVE 멤버에게 모임장 권한을 넘기고 본인은 일반 멤버로 강등할 수 있습니다.")
    ...
    @Operation(
            summary = "중도 하차",
            description = "멤버가 그룹에서 하차합니다. 모집 중에는 PLANNED 배정을 다시 판단합니다. " +
                    "진행 중에는 현재 라운드를 완독한 경우 기존 미래 라운드를 보존·비활성화하고 읽기 순서 재확인 상태로 전환합니다."
    )
    @DeleteMapping("/me")
