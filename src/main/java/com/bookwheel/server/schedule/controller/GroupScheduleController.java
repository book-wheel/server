...
public class GroupScheduleController {
    ...
    @Operation(
            ...
            description = "그룹의 전체 라운드 일정과 현재 ACTIVE 멤버 기준 실행 범위, READY 차단 사유 및 로그인한 사용자의 책 배정을 조회합니다. " +
                    "진행 중 멤버 변동이 있으면 scheduleReconfigurationStatus로 모임장 또는 부모임장의 다음 재확정 단계를 안내합니다. " +
                    "모임 관리자 화면은 scheduleStatus가 NOT_CONFIGURED/CONFIGURED/READY/RESCHEDULE_REQUIRED이면 필요할 때 POST /schedule/preview로 확인한 뒤 POST /schedule로 확정하고, " +
    ...
    @Operation(
            summary = "독서 일정 생성",
            description = "ACTIVE 모임장(LEADER) 또는 부모임장(SUB_LEADER)이 모집 중(RECRUITING)인 모임에 모임 최대 인원 기준 라운드 날짜 틀을 저장합니다. " +
                    "현재 인원이나 도서 등록 여부와 관계없이 전체 틀은 생성합니다. ACTIVE 멤버가 2명 이상이고 " +
                    "전원이 책을 등록해 현재 인원 기준 N-1개 라운드 배정이 준비되면 최대 인원을 다 채우지 않아도 READY를 반환합니다."
    )
    @ApiResponses({
            ...
            @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "403", description = "모임장/부모임장 권한 없음 (GROUP_057)")
    })
    ...
    @Operation(
            summary = "독서 일정 미리보기",
            description = "ACTIVE 모임장(LEADER) 또는 부모임장(SUB_LEADER)이 POST /schedule과 같은 요청으로 모임 최대 인원 기준 라운드 날짜를 계산하지만 DB에는 저장하지 않습니다. " +
    ...
    @Operation(
            ...
            description = "진행 중(IN_PROGRESS)인 모임에서 오늘 시작했거나 이미 시작된 라운드는 보존하고 미래 라운드만 재생성합니다. " +
                    "ACTIVE 모임장(LEADER) 또는 부모임장(SUB_LEADER)이 미래 일정을 최종 확정할 수 있습니다. " +
