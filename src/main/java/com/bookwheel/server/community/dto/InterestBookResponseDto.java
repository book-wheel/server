...
public record InterestBookResponseDto(
    ...
    @Schema(description = "도서 제목 (등록 시점에 저장된 도서 정보가 없으면 null)",
        example = "달러구트 꿈 백화점", nullable = true)
    String title,

    @Schema(description = "저자 (등록 시점에 저장된 도서 정보가 없으면 null)", example = "이미예", nullable = true)
    String author,

    @Schema(description = "도서 표지 이미지 URL (등록 시점에 저장된 도서 정보가 없으면 null)",
