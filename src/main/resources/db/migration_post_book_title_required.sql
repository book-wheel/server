...
--
-- 실행 시점: 배포 전에 기존 게시글 데이터를 삭제할 예정이면 아래 순서대로 데이터를 먼저 비운 뒤,
--           애플리케이션 배포 또는 ALTER TABLE을 진행한다.
-- 대상 DB : dev, prod 각각
--
-- 삭제 대상은 post를 참조하는 게시글 데이터와 커뮤니티 알림이다.
-- book_info, book_like, book_review, book_vote, review_like는 삭제 대상이 아니다.
-- users, reading_group, member, book, own_book 등 사용자/모임/모임도서 데이터도 삭제 대상이 아니다.
